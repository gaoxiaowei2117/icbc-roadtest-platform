...
| T3 | 无单元测试 / 集成测试 | 加 `pytest` 套件覆盖：JWT 签发、Fernet 往返、auth flow、claim 原子性、admin 鉴权 |
| T4 | `ENCRYPTION_KEY` 无轮换机制：换密钥后所有存量 `secret.ciphertext` 失效 | 短期：文档化警告 + 数据库备份；长期：双密钥过渡（`key_id` + 多 Fernet 实例） |
...
| ID | 决策 | 理由 |
|---|---|---|
| A1 | 凭据 Fernet 加密存 DB，主密钥只放 worker 的 `.env`，云端拿不到 | 假设 VPS 被攻破，攻击者拿不到 ICBC 凭据明文 |
