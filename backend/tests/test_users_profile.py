def test_update_and_read_profile(client, auth_headers):
    ...
    assert body["icbc_original_email"] == "driver@example.com"
    ...
    assert me["icbc_original_email"] == "driver@example.com"
    assert me["expect_after_date"] == "2026-07-01"


def test_original_email_must_be_valid(client, auth_headers):
    h = auth_headers()
    r = client.patch(
        "/api/users/me",
        headers=h,
        json={"icbc_original_email": "not-an-email"},
    )
